        ...
        <div class="bg-white shadow-md rounded-lg p-20 max-w-xl w-full">
            ...
            <form id="searchForm" method="get" data-url="{{ route('search_events_by_student') }}">
                <div class="flex items-center mb-4 flex-col gap-5">
                    <input id="studentId"
                        class="input-search flex-1 p-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        name="student_id" placeholder="Nhập mã số sinh viên" type="text">
                    <button type="submit" class="btn-search p-3 bg-blue-800 text-white rounded-lg hover:bg-blue-900"><i
                            class="fa-solid fa-magnifying-glass"></i> Tìm kiếm</button>
                </div>
            </form>
            <!-- Modal -->
            <div id="eventsModal"
                class="fixed inset-0 z-[1000] flex items-center justify-center bg-black bg-opacity-50 hidden">
                <div class="bg-white rounded-lg w-full sm:max-w-lg lg:max-w-4xl overflow-hidden">
                    <div class="flex justify-between items-center px-6 py-4 bg-blue-950">
                        ...
                        <button class="text-gray-200 hover:text-yellow-400 focus:outline-none p-5 text-2xl"
        ...
        document.getElementById("searchForm").addEventListener("submit", function(event) {
            ...
            function showEvents(data) {
                ...
                modalTitle.innerHTML =
                    `Kết quả tra cứu cho sinh viên ${data.student.fullname} (MSSV: ${data.student.id})`;
